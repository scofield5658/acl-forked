...
MongoDBBackend.prototype = {
  ...
  */
  get(bucket, key, cb) {
    ...
    try {
      const collection = this.db.collection(this.prefix + this.removeUnsupportedChar(collName));
      ...
      collection.findOne(searchParams, { _bucketname: 0 }, (err, doc) => {
        ...
        doc = fixKeys(doc);
        return cb(undefined, _.without(_.keys(doc), 'key', '_id'));
      });
    } catch (error) {
      cb(error);
    }
  ...
  */
  add(transaction, bucket, key, values) {
    ...
    transaction.push((cb) => {
      ...
      try {
        const collection = self.db.collection(self.prefix + self.removeUnsupportedChar(collName));
        // build doc from array values
        const doc = {};
        values.forEach((value) => { doc[value] = true; });

        // update document
        collection.updateOne(updateParams, { $set: doc }, { safe: true, upsert: true }, (err) => {
          if (err instanceof Error) return cb(err);
          return cb(undefined);
        });
      } catch (error) {
  ...
  remove(transaction, bucket, key, values) {
    ...
    transaction.push((cb) => {
      try {
        ...
        // update document
        collection.updateOne(updateParams, { $unset: doc }, { safe: true, upsert: true }, (err) => {
          if (err instanceof Error) return cb(err);
          return cb(undefined);
        });
      } catch (error) {
