...
Acl.prototype.checkRole = function (roles, cb) {
  ...
  return this.backend.getAsync(this.options.buckets.meta, 'roles').then((real_roles) => {
    const result = {};
    roles.forEach((role) => {
      result[role] = real_roles.includes(role);
    });
    return result;
  }).nodeify(cb);
};
...
Acl.prototype._checkPermissions = function (roles, resource, permissions) {
  ...
  return this.backend.unionAsync(allowsBucket(resource), roles).then((resourcePermissions) => {
    ...
    return that.backend.unionAsync(that.options.buckets.parents, roles).then((parents) => {
      if (parents && parents.length) {
        return that._checkPermissions(parents, resource, permissions);
      }
      return false;
    });
  });
